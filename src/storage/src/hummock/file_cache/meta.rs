...
use super::coding::{CacheKey, CacheValue};
use super::error::Result;

#[derive(PartialEq, Eq, Debug)]
pub struct BlockLoc {
    /// cache file id, set 0 if invalid
    pub fid: u32,
    ...
    V: CacheValue,
{
    ...
    pub fn set(&mut self, slot: usize, bloc: &BlockLoc, key: &K) {
        debug_assert!(
            slot < self.slots,
            "slot: {}, self.slots: {}",
            slot,
    ...
    pub fn get(&self, slot: usize) -> Option<(BlockLoc, K)> {
        debug_assert!(
            slot < self.slots,
            "slot: {}, self.slots: {}",
            slot,
...
impl<K, V> Drop for MetaFile<K, V>
where
    K: CacheKey,
    V: CacheValue,
{
...
mod tests {
    ...
    struct TestCacheKey(u64);

    #[derive(Debug, PartialEq, Eq)]
    struct TestCacheValue(u64);
    ...
    impl CacheValue for TestCacheValue {}
    ...
    fn test_enc_dec() {
        ...
        let mut mf: MetaFile<TestCacheKey, TestCacheValue> =
        ...
        let mf: MetaFile<TestCacheKey, TestCacheValue> =
            MetaFile::open(dir.path().join("test-meta-001"), SIZE).unwrap();
        let (bloc0, key0) = mf.get(0).unwrap();
        assert_eq!(bloc0, bloc);
        assert_eq!(key0, key);
